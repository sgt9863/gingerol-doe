...
def run_fit_and_designspace(df, header_prefix=""):
    ...
    st.caption("R²=当てはまり、RMSE=予測の平均的なズレ（小さいほど良い）。"
    ...
    c2.success(
        ...
        f"- T = {rec['T']:.1f} ℃\n- φ = {rec['phi']:.3f}（ACN 分率）\n"
        f"- F = {rec['F']:.2f} mL/min\n\n"
        f"→ Rs_min={float(s['Rs_min']):.2f} / t_R(TP)={float(s['TP']['tR']):.2f}分 / 最遅={last:.2f}分"
    )
    ...
    rec_df = pd.DataFrame([{
        ...
        "tR_TP_min": float(s["TP"]["tR"]), "tR_last_min": last,
    }])
...
with tab1:
    ...
    st.write("中心複合計画（頂点8＋軸上6＋中心点）を生成します。Excel 雛形をDLし、"
             "各条件で測った 3 ピークの t_R・W_h を空欄に記入してください。")
    ccd = design.ccd_design(factors, n_center=n_center, alpha=1.0, day=0).copy()
    ccd.insert(0, "run", np.arange(1, len(ccd) + 1))
    for col in design.RESPONSE_COLS:
        ccd[col] = np.nan
    st.dataframe(ccd[["run", "day", "type", "T", "phi", "F"]], use_container_width=True)
    st.caption(f"Day1 = {len(ccd)} 本（中心点 {n_center}）。1注入で3ピーク全部測れます。")
