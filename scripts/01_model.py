...
1ピークのパラメータは dict で持つ:
  retention（保持）: a, b, c, d, delta
    ln k = a + b/T_K + c*phi + d*phi^2 + delta*day   （T_K = T+273.15 [K]）
      a     : 切片
...
# 保持（retention）
# ──────────────────────────────
def retention_factor(T, phi, a, b, c, d=0.0, day=0, delta=0.0):
    """保持係数 k を返す。 ln k = a + b/T_K + c*phi + d*phi^2 + delta*day"""
    T_K = np.asarray(T, dtype=float) + KELVIN
    ln_k = a + b / T_K + c * phi + d * np.asarray(phi, dtype=float) ** 2 + delta * day
    return np.exp(ln_k)
...
def retention_time(T, phi, F, Vm, a, b, c, d=0.0, day=0, delta=0.0):
    """保持時間 t_R = (Vm/F)*(1+k) [min]。"""
    k = retention_factor(T, phi, a, b, c, d, day, delta)
    return (Vm / F) * (1.0 + k)
...
def predict_peak(peak, T, phi, F, Vm, L_mm, day=0):
    ...
    tR = retention_time(
        ...
        peak.get("d", 0.0), day, peak.get("delta", 0.0),
    )
...
def example_peaks():
    ...
    return {
        "IP1": {"a": -1.39, "b": 1800.0, "c": -3.2, "d": 0.0, "delta": 0.0, **vd},
        "TP":  {"a": -0.49, "b": 1500.0, "c": -3.0, "d": 0.0, "delta": 0.0, **vd},
        "IP2": {"a":  0.10, "b": 1300.0, "c": -2.8, "d": 0.0, "delta": 0.0, **vd},
    }
