...
現状は **設計フェーズ完了・実装これから**。`scripts/` は空（README のみ）。実データは社外（会社）に
あり本リポジトリには無いため、実装は **合成（ダミー）データで動作確認**してから引き渡す前提。
...
- **デザインスペース**：`min{Rs1,Rs2} ≥ 2.0` かつ `t_R(TP) ≤ 7.5min` かつ `max(t_R 全3本) ≤ 10min`
  （クロスオーバーで最遅ピークが入れ替わるため、洗浄前制約は最遅ピークに課す）。
...
- `V_m` は幾何推算 `≈ 0.68 × π r² L ≈ 0.24 mL`（2.1×100mm）で足りる。厳密化は不保持物質注入。
...
```
config.example.yaml  設定の実体（因子範囲・ピーク定義・Vm・合格条件・design設定）— 未作成
scripts/01_model.py  数理モデル（k, W, Rs）          — 未実装
scripts/02_design.py CCD生成＋ブロッキング            — 未実装
scripts/03_fit.py    フィット（保持3・幅3・day推定）  — 未実装
scripts/04_optimize.py デザインスペース判定＋最大余裕点 — 未実装
scripts/05_designspace.py plotly 3D                  — 未実装
app.py               Streamlit Web アプリ（薄い入口） — 未実装
```
