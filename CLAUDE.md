...
現状は **設計フェーズ確定・実装一巡完了・skill 化も完了**。config.example.yaml ＋ scripts/01〜05 ＋
app.py（Streamlit・タブ式の正規フロー）が合成データで動作確認済み。再利用パッケージは
`.claude/skills/hplc-design-space/` に自己完結で切り出し済み（別化合物は config.yaml 差し替えで転用）。
実データは社外（会社）にあり本リポジトリには無いため、**合成（ダミー）データで動作確認**してから引き渡す前提。
